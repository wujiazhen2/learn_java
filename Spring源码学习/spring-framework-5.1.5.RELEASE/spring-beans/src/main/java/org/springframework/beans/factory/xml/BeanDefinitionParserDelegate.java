...
public class BeanDefinitionParserDelegate {
	 ...
	 * 设置<beans/>内部bean的默认属性，lazy-init,autowire, dependency check settings, init-method, destroy-method
	...
	public BeanDefinitionHolder parseBeanDefinitionElement(Element ele, @Nullable BeanDefinition containingBean) {
		...
		if (StringUtils.hasLength(nameAttr)) {
			//bean标签的name属性可以通过MULTI_VALUE_ATTRIBUTE_DELIMITERS分割符配置多个。
	...
	public AbstractBeanDefinition parseBeanDefinitionElement(
		...
		try {
			...
			//<bean/> lookup-method 子标签解析
			parseLookupOverrideSubElements(ele, bd.getMethodOverrides());
			//<bean/> replace-method 子标签解析
			parseReplacedMethodSubElements(ele, bd.getMethodOverrides());
			//<bean/> constructor-arg 构造函数
			parseConstructorArgElements(ele, bd);
			//<bean/> property 构造函数
			parsePropertyElements(ele, bd);
			parseQualifierElements(ele, bd);
			//为当前解析的Bean设置依赖的资源和标签element对象
